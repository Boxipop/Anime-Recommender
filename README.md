...
The features were then vectorized so that the KNN model would be able to effectively use the data and do the math. I used Tfidf for the synopsis and tags (genres, themes, and demographcis) and One Hot Encoder for anime type and studio.

The cosine distance metric was used in my KNN model as I didn't want my results to be influenced by any magntidue differences in data. 

## Making the Recommendation System Interactive 
In order to make my recommendation system easily accessible and interactive, I built and deployed a web application using Streamlit. On the app, the user would input an anime he or she likes. Then, the model would calculate the top 10 most similar anime (according to the features and their weights) in the multidimensional vector space. The app will then display the title of the suggested anime along with their cover art (obtained through Jikan) in a 2x5 grid.
